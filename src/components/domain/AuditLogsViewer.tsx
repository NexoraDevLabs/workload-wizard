...
export function AuditLogsViewer({
  ...
  const formatEntityTypeLabel = (entityType: string) =>
    formatWords(entityType, /[_]/g);

  const loadLogs = useCallback(
    async (page = 1, cursor?: string | null, reset = false) => {
      setIsLoading(true);
      setError(null);
      try {
        const now = Date.now();
        const defaultStart = now - filters.timeRange * 60 * 60 * 1000;
        const args = {
          ...(filters.entityType ? { entityType: filters.entityType } : {}),
          ...(filters.action ? { action: filters.action } : {}),
          ...(filters.severity ? { severity: filters.severity } : {}),
          ...(forcedFilters?.type
            ? { type: forcedFilters.type }
            : filters.type
              ? { type: filters.type }
              : {}),
          ...(forcedFilters?.organisationId
            ? { organisationId: forcedFilters.organisationId }
            : {}),
          ...(filters.search ? { search: filters.search } : {}),
          ...(filters.timeRange > 0 ? { startDate: defaultStart } : {}),
          ...(cursor ? { cursor } : {}),
